...
def main():
    ...
    try:
        ...
        test_model = ModelInterface(model_type="res2", use_attention=True)
        test_model.load_checkpoint("/home/nymath/dev/rl/gomoku/model/ResNet2-gomuku-2023-12-24-11-23-iteration-17000.pth")
        
        if args.checkpoint:
            model.load_checkpoint(args.checkpoint)
            
        mcts_player = MCTSZeroAgent(model.forward, player_id=1, c_puct=5, n_playout=train_args["n_playout"])
        ...
        for i in tqdm.tqdm(range(train_args['n_iterations'])):
            ...
            train_args["batch_size"] = len(buffer)
            ...
            with open("rllog.txt", "a+") as f:
                f.write(line)
                
            # if (i + 1) % train_args["log_freq"] == 0:
            valid_mcts_player = MCTSZeroAgent(model.forward, player_id=1, n_playout=800)
            test_mcts_player = MCTSZeroAgent(test_model.forward, player_id=2, n_playout=800)
            winners = [0, 0, 0]
            for _ in range(5):
...
def create_args():
    ...
    project_args = {
        "run_name": datetime.datetime.now().strftime("rl-%Y-%m-%d-%H-%M"),
        "project_name": "ResNet5",
        "save_dir": "./rl_model",
    }
    train_args.update({
        "log_freq": 1,
